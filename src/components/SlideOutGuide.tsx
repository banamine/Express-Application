...
function useLiveThumbnail(segmentStart: number, segmentDuration: number, baseThumbUrl: string) {
  const [thumbSrc, setThumbSrc] = useState(`${baseThumbUrl}/frame_0001.jpg`);

  const intervalRef1 = useRef<ReturnType<typeof setInterval> | null>(null);
  useEffect(() => {
    const updateImage = () => {
      const now = Date.now() / 1000;
      if (now >= segmentStart && now <= segmentStart + segmentDuration) {
        const elapsed = now - segmentStart;
        const percentComplete = elapsed / segmentDuration;
        
        // Assuming Archive.org generates 60 frames per video
        const frameNumber = Math.max(1, Math.floor(percentComplete * 60));
        
        // Pad with zeros (e.g., 0014)
        const paddedFrame = String(frameNumber).padStart(4, '0');
        
        // e.g., https://archive.org/download/item/item.thumbs/frame_0014.jpg
        setThumbSrc(`${baseThumbUrl}/frame_${paddedFrame}.jpg`);
      }
    };

    updateImage(); // Run once immediately
...
function ThumbnailImage({ segmentStart, segmentDuration, baseThumbUrl, alt }: { segmentStart: number, segmentDuration: number, baseThumbUrl: string, alt: string }) {
  const thumbSrc = useLiveThumbnail(segmentStart, segmentDuration, baseThumbUrl);
  return (
    <img 
      src={thumbSrc} 
      alt={alt}
